...
class CrashActivity : BaseActivity<ActivityCrashBinding>(), View.OnClickListener {
    ...
    override fun onClick(v: View) {
        ...
        when (v.id) {
            ...
            R.id.btn_crash_log -> {
                if (mDialog == null) {
                    mDialog = AlertDialog.Builder(this@CrashActivity).setTitle(R.string.crash_error_details).setMessage(CustomActivityOnCrash.getAllErrorDetailsFromIntent(this@CrashActivity, intent)).setPositiveButton(
                        R.string.crash_close, null).setNeutralButton(R.string.crash_copy_log) { _: DialogInterface?, _: Int -> copyErrorToClipboard() }.create()
                }
                mDialog!!.show()
    ...
    private fun copyErrorToClipboard() {
        ...
        (getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager).setPrimaryClip(ClipData.newPlainText(getString(
            R.string.crash_error_info
        ), errorInformation))
    }
